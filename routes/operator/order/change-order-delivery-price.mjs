...
import {changeOrderDeliveryPrice, getCourierUniqueId} from "../../../modules/query/operator-query.mjs";
                       ...
                       .then(async result_courier=>{
                           if(result_courier.rows.length){
                               await sendMessage(result_courier.rows[0].courier_unique_id,
                                   `Sargydyň eltip berilmeli bahasy üýtgedi!`,
                                   `Täze bahasy: ${delivery_price}, ${reason}`,
                                   {
