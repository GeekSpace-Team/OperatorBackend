...
import {
    ...
    getAllOrderLocation,
    getAllOrderProduct,
    getAllOrderProductStatus, getAllOrderStatus, getAllPhoneCalls
...
getAllOrders.get('/', verifyToken, async (req, res) => {
    ...
    await db.query(`${format(getAllCustomerOrders, sell_point_id)} ${format(getAllCustomerOrderAddress, sell_point_id)}
                ...
                ${format(getAllPhoneCalls, sell_point_id)} ${format(getAllOrderStatus, sell_point_id)} `)
        .then(results => {
            res.json(response(false, 'success', {
                ...
                phone_calls: results[8].rows,
                order_status: results[9].rows,
            }));
            res.end();
        })
        .catch(err => {
