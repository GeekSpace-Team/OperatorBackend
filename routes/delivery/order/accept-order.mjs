    ...
    if (typeof req.body === 'undefined' || req.body == null) {
        ...
        if (typeof orders !== 'undefined' && orders != null && orders.length > 0) {
            orders.forEach((item, i) => {
                orderValues.push([item.unique_id, orderStatus.COURIER_ACCEPTED, reason, req.user.user.unique_id, 'now()', 'now()']);
            });
                ...
                .then(async result => {
                    if (result.rows.length) {
                        ...
                        orderProducts.forEach((item) => {
                            orderProductValues.push([item.unique_id, orderStatus.COURIER_ACCEPTED, req.user.user.unique_id, 'now()', 'now()', reason]);
                        });
